...
var require_utils$2 = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	...
	var sharedArrayBufferByteLengthGetter = Object.getOwnPropertyDescriptor(SharedArrayBuffer.prototype, "byteLength").get;
	function isSharedArrayBuffer(value) {
		try {
...
var require_percent_encoding = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	...
	var extraQueryPercentEncodeChars = String.fromCodePoint(...[...extraQueryPercentEncodeSet].sort());
	function isQueryPercentEncode(c) {
		return isC0ControlPercentEncode(c) || extraQueryPercentEncodeSet.has(c);
	}
	var extraSpecialQueryPercentEncodeSet = new Set([...extraQueryPercentEncodeSet, p("'")]);
	var extraSpecialQueryPercentEncodeChars = String.fromCodePoint(...[...extraSpecialQueryPercentEncodeSet].sort());
...
var require_url_state_machine = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	init_encoding();
	var { percentEncodeAfterEncoding } = (init_whatwg(), __toCommonJS(whatwg_exports));
	var tr46 = require_tr46();
	var infra = require_infra();
	var { utf8DecodeWithoutBOM } = require_encoding();
	var { percentDecodeString, utf8PercentEncodeCodePoint, utf8PercentEncodeString, isC0ControlPercentEncode, isFragmentPercentEncode, extraQueryPercentEncodeChars, extraSpecialQueryPercentEncodeChars, isPathPercentEncode, isUserinfoPercentEncode } = require_percent_encoding();
	function p(char) {
		return char.codePointAt(0);
	}
	var specialSchemes = {
...
//#endregion
//#region node_modules/whatwg-url/index.js
...
//#endregion
//#region node_modules/data-urls/node_modules/whatwg-mimetype/lib/utils.js
...
//#endregion
//#region node_modules/data-urls/node_modules/whatwg-mimetype/lib/mime-type-parameters.js
var require_mime_type_parameters = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	var { asciiLowercase, solelyContainsHTTPTokenCodePoints, soleyContainsHTTPQuotedStringTokenCodePoints } = require_utils$1();
	module.exports = class MIMETypeParameters {
		...
		set(name, value) {
			name = asciiLowercase(String(name));
			value = String(value);
			if (!solelyContainsHTTPTokenCodePoints(name)) throw new Error(`Invalid MIME type parameter name "${name}": only HTTP token code points are valid.`);
			if (!soleyContainsHTTPQuotedStringTokenCodePoints(value)) throw new Error(`Invalid MIME type parameter value "${value}": only HTTP quoted-string token code points are valid.`);
...
//#endregion
//#region node_modules/data-urls/node_modules/whatwg-mimetype/lib/parser.js
var require_parser$1 = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	var { removeLeadingAndTrailingHTTPWhitespace, removeTrailingHTTPWhitespace, isHTTPWhitespaceChar, solelyContainsHTTPTokenCodePoints, soleyContainsHTTPQuotedStringTokenCodePoints, asciiLowercase, collectAnHTTPQuotedString } = require_utils$1();
	module.exports = (input) => {
		...
		if (type.length === 0 || !solelyContainsHTTPTokenCodePoints(type)) return null;
		...
		if (subtype.length === 0 || !solelyContainsHTTPTokenCodePoints(subtype)) return null;
		const mimeType = {
		...
		while (position < input.length) {
			++position;
			while (isHTTPWhitespaceChar(input[position])) ++position;
			let parameterName = "";
			while (position < input.length && input[position] !== ";" && input[position] !== "=") {
			...
			if (parameterName.length > 0 && solelyContainsHTTPTokenCodePoints(parameterName) && soleyContainsHTTPQuotedStringTokenCodePoints(parameterValue) && !mimeType.parameters.has(parameterName)) mimeType.parameters.set(parameterName, parameterValue);
		}
		return mimeType;
	};
}));
//#endregion
//#region node_modules/data-urls/node_modules/whatwg-mimetype/lib/serializer.js
var require_serializer = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	var { solelyContainsHTTPTokenCodePoints } = require_utils$1();
	module.exports = (mimeType) => {
		let serialization = `${mimeType.type}/${mimeType.subtype}`;
		if (mimeType.parameters.size === 0) return serialization;
...
//#endregion
//#region node_modules/data-urls/node_modules/whatwg-mimetype/lib/mime-type.js
var require_mime_type = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	var MIMETypeParameters = require_mime_type_parameters();
	var parse = require_parser$1();
	var serialize = require_serializer();
	var { asciiLowercase, solelyContainsHTTPTokenCodePoints } = require_utils$1();
	module.exports = class MIMEType {
		constructor(string) {
			string = String(string);
			const result = parse(string);
			if (result === null) throw new Error(`Could not parse MIME type string "${string}"`);
		...
		set type(value) {
			...
			if (value.length === 0) throw new Error("Invalid type: must be a non-empty string");
			if (!solelyContainsHTTPTokenCodePoints(value)) throw new Error(`Invalid type ${value}: must contain only HTTP token code points`);
			this._type = value;
		}
		get subtype() {
		...
		set subtype(value) {
			value = asciiLowercase(String(value));
			if (value.length === 0) throw new Error("Invalid subtype: must be a non-empty string");
			if (!solelyContainsHTTPTokenCodePoints(value)) throw new Error(`Invalid subtype ${value}: must contain only HTTP token code points`);
			this._subtype = value;
		}
		get parameters() {
		...
		isJavaScript({ prohibitParameters = false } = {}) {
			switch (this._type) {
				case "text": switch (this._subtype) {
					case "ecmascript":
					case "javascript":
					case "javascript1.0":
					case "javascript1.1":
				...
				case "application": switch (this._subtype) {
					case "ecmascript":
					case "javascript":
					case "x-ecmascript":
					case "x-javascript": return !prohibitParameters || this._parameters.size === 0;
...
//#endregion
//#region node_modules/data-urls/node_modules/whatwg-mimetype/lib/sniff.js
var require_sniff = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	var MIMEType = require_mime_type();
	function normalizeMIMEType(input) {
		return MIMEType.parse(`${input}`);
	}
	function isXMLMIMEType(mimeType) {
		return mimeType.subtype.endsWith("+xml") || mimeType.type === "text" && mimeType.subtype === "xml" || mimeType.type === "application" && mimeType.subtype === "xml";
	}
	function isHTMLMIMEType(mimeType) {
		return mimeType.type === "text" && mimeType.subtype === "html";
	}
	var RESOURCE_HEADER_LENGTH = 1445;
	function getResourceHeader(resource) {
		if (resource.length <= RESOURCE_HEADER_LENGTH) return resource;
	...
	function isAudioOrVideoMIMEType(mimeType) {
		return mimeType.type === "audio" || mimeType.type === "video" || mimeType.type === "application" && mimeType.subtype === "ogg";
	}
	...
	function matchesSignature(resource, signature) {
		...
		let s = 0;
		if (ignoredLeadingBytes) while (s < resource.length && ignoredLeadingBytes(resource[s])) s++;
		if (resource.length < s + pattern.length) return null;
		for (let i = 0; i < pattern.length; i++) if ((resource[s + i] & mask[i]) !== (pattern[i] & mask[i])) return null;
		return mimeType;
	}
	var step1Table = [
		{
	...
	function matchMP4(resource) {
		...
		if (resource[4] !== 102 || resource[5] !== 116 || resource[6] !== 121 || resource[7] !== 112) return null;
		const length = resource[0] << 24 | resource[1] << 16 | resource[2] << 8 | resource[3];
		if (length < 12 || length > resource.length) return null;
		...
		for (let i = 16; i + 4 <= length && i + 4 <= resource.length; i += 4) {
			...
			if (mp4Brands.includes(compat)) return "video/mp4";
		}
		return null;
	}
	...
	function matchPaddedSequence(sequence, offset, pattern) {
		while (offset < sequence.length && sequence[offset] === 0) offset++;
		if (sequence.length < offset + pattern.length) return false;
		for (let i = 0; i < pattern.length; i++) if (sequence[offset + i] !== pattern[i]) return false;
		return true;
	}
	function matchWebM(resource) {
		const { length } = resource;
		if (length < 4) return null;
		if (resource[0] !== 26 || resource[1] !== 69 || resource[2] !== 223 || resource[3] !== 163) return null;
		let iter = 4;
		while (iter < length && iter < 38) {
			if (iter + 1 < length && resource[iter] === 66 && resource[iter + 1] === 130) {
				...
				iter += numberSize;
				if (iter >= length - 4) break;
				if (matchPaddedSequence(resource, iter, [
					119,
					101,
					98,
					109
	...
	function matchMP3Header(sequence, s) {
		...
		if (sequence[s] !== 255 || (sequence[s + 1] & 224) !== 224) return false;
		const layer = (sequence[s + 1] & 6) >> 1;
		if (layer === 0) return false;
		if ((sequence[s + 2] & 240) >> 4 === 15) return false;
		if ((sequence[s + 2] & 12) >> 2 === 3) return false;
		if ((4 - layer & 3) !== 3) return false;
		return true;
	}
	function parseMP3Frame(sequence, s) {
		...
		return {
			version,
			bitrate: (version & 1) !== 0 ? mp3Rates[bitrateIndex] : mp25Rates[bitrateIndex],
			samplerate: sampleRates[(sequence[s + 2] & 12) >> 2],
			pad: (sequence[s + 2] & 2) >> 1
		};
	}
	...
	function matchMP3WithoutID3(resource) {
		const { length } = resource;
		let s = 0;
		if (!matchMP3Header(resource, s)) return null;
		...
		if (skippedBytes < 4 || skippedBytes > length - s) return null;
		s += skippedBytes;
		if (!matchMP3Header(resource, s)) return null;
		return "audio/mpeg";
	}
	...
	function distinguishTextOrBinary(resourceHeader) {
		...
		if (length >= 2) {
			if (resourceHeader[0] === 254 && resourceHeader[1] === 255) return "text/plain";
			if (resourceHeader[0] === 255 && resourceHeader[1] === 254) return "text/plain";
		}
		if (length >= 3) {
			if (resourceHeader[0] === 239 && resourceHeader[1] === 187 && resourceHeader[2] === 191) return "text/plain";
		}
		for (let i = 0; i < length; i++) if (isBinaryDataByte(resourceHeader[i])) return "application/octet-stream";
		return "text/plain";
	}
	function identifyAnUnknownMIMEType(resourceHeader, { sniffScriptable = false } = {}) {
		if (sniffScriptable) for (const sig of step1Table) {
		...
		for (let i = 0; i < resourceHeader.length; i++) if (isBinaryDataByte(resourceHeader[i])) return "application/octet-stream";
		return "text/plain";
	}
	var apacheBugValues = new Set([
		"text/plain",
	...
	function detectSuppliedMIMEType({ contentTypeHeader, providedType }) {
		...
		if (contentTypeHeader !== void 0) {
			...
			if (suppliedMIMEType !== null && typeof contentTypeHeader === "string") checkForApacheBug = apacheBugValues.has(contentTypeHeader);
		} else if (providedType !== void 0) suppliedMIMEType = normalizeMIMEType(providedType);
		return {
			suppliedMIMEType,
			checkForApacheBug
		};
	...
	*/
	module.exports = function computedMIMEType(resource, { contentTypeHeader, providedType, noSniff = false, isSupported = () => true } = {}) {
		const resourceHeader = getResourceHeader(resource);
		const { suppliedMIMEType, checkForApacheBug } = detectSuppliedMIMEType({
			contentTypeHeader,
			providedType
		});
		if (suppliedMIMEType !== null && (isXMLMIMEType(suppliedMIMEType) || isHTMLMIMEType(suppliedMIMEType))) return suppliedMIMEType;
		if (suppliedMIMEType === null || suppliedMIMEType.essence === "unknown/unknown" || suppliedMIMEType.essence === "application/unknown" || suppliedMIMEType.essence === "*/*") return new MIMEType(identifyAnUnknownMIMEType(resourceHeader, { sniffScriptable: !noSniff }));
		if (noSniff) return suppliedMIMEType;
		if (checkForApacheBug) return new MIMEType(distinguishTextOrBinary(resourceHeader));
		if (isImageMIMEType(suppliedMIMEType) && isSupported(suppliedMIMEType)) {
			const imageResult = matchImageType(resourceHeader);
			if (imageResult !== null) return new MIMEType(imageResult);
		}
		if (isAudioOrVideoMIMEType(suppliedMIMEType) && isSupported(suppliedMIMEType)) {
			...
			if (avResult !== null) return new MIMEType(avResult);
		}
		return suppliedMIMEType;
	};
}));
//#endregion
//#region node_modules/data-urls/node_modules/whatwg-mimetype/lib/index.js
var require_lib = /* @__PURE__ */ __commonJSMin(((exports) => {
	exports.MIMEType = require_mime_type();
	exports.computedMIMEType = require_sniff();
}));
//#endregion
//#region node_modules/data-urls/lib/utils.js
...
//#endregion
//#region node_modules/data-urls/lib/parser.js
var require_parser = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	var { MIMEType } = require_lib();
	var { parseURL, serializeURL, percentDecodeString } = require_whatwg_url();
	var { stripLeadingAndTrailingASCIIWhitespace, isomorphicDecode, forgivingBase64Decode } = require_utils();
	...
	module.exports.fromURLRecord = (urlRecord) => {
		...
		if (position === input.length) return null;
		++position;
		let body = percentDecodeString(input.substring(position));
...
//#endregion
export { require_lib$1 as i, require_whatwg_url as n, require_webidl2js_wrapper as r, require_parser as t };
