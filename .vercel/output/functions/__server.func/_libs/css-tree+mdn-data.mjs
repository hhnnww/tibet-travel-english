...
import { r as require_source_map_generator } from "./@asamuzakjp/dom-selector+[...].mjs";
//#region node_modules/css-tree/cjs/tokenizer/types.cjs
...
//#endregion
//#region node_modules/css-tree/cjs/tokenizer/char-code-definitions.cjs
var require_char_code_definitions = /* @__PURE__ */ __commonJSMin(((exports) => {
	var EOF = 0;
...
//#endregion
//#region node_modules/css-tree/cjs/tokenizer/utils.cjs
var require_utils = /* @__PURE__ */ __commonJSMin(((exports) => {
	...
	function cmpChar(testStr, offset, referenceCode) {
		...
		if (charCodeDefinitions.isUppercaseLetter(code)) code = code | 32;
	...
	function cmpStr(testStr, start, end, referenceStr) {
		...
		for (let i = start; i < end; i++) {
			...
			if (charCodeDefinitions.isUppercaseLetter(testCode)) testCode = testCode | 32;
			if (testCode !== referenceCode) return false;
		}
		return true;
	}
	function findWhiteSpaceStart(source, offset) {
		for (; offset >= 0; offset--) if (!charCodeDefinitions.isWhiteSpace(source.charCodeAt(offset))) break;
		return offset + 1;
	}
	function findWhiteSpaceEnd(source, offset) {
		for (; offset < source.length; offset++) if (!charCodeDefinitions.isWhiteSpace(source.charCodeAt(offset))) break;
		return offset;
	}
	function findDecimalNumberEnd(source, offset) {
		for (; offset < source.length; offset++) if (!charCodeDefinitions.isDigit(source.charCodeAt(offset))) break;
		return offset;
	}
	function consumeEscaped(source, offset) {
		offset += 2;
		if (charCodeDefinitions.isHexDigit(getCharCode(source, offset - 1))) {
			for (const maxOffset = Math.min(source.length, offset + 5); offset < maxOffset; offset++) if (!charCodeDefinitions.isHexDigit(getCharCode(source, offset))) break;
			const code = getCharCode(source, offset);
			if (charCodeDefinitions.isWhiteSpace(code)) offset += getNewlineLength(source, offset, code);
		}
		return offset;
	}
	function consumeName(source, offset) {
		for (; offset < source.length; offset++) {
			...
			if (charCodeDefinitions.isName(code)) continue;
	...
	function consumeNumber(source, offset) {
		...
		if (cmpChar(source, offset, 101)) {
			...
			if (charCodeDefinitions.isDigit(code)) offset = findDecimalNumberEnd(source, offset + 1 + sign + 1);
		}
		return offset;
	}
	function consumeBadUrlRemnants(source, offset) {
		for (; offset < source.length; offset++) {
			...
			if (charCodeDefinitions.isValidEscape(code, getCharCode(source, offset + 1))) offset = consumeEscaped(source, offset);
		}
		return offset;
	}
	function decodeEscaped(escaped) {
		if (escaped.length === 1 && !charCodeDefinitions.isHexDigit(escaped.charCodeAt(0))) return escaped[0];
...
//#endregion
//#region node_modules/css-tree/cjs/tokenizer/names.cjs
var require_names$1 = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	module.exports = [
		"EOF-token",
		"ident-token",
		"function-token",
...
//#endregion
//#region node_modules/css-tree/cjs/tokenizer/adopt-buffer.cjs
var require_adopt_buffer = /* @__PURE__ */ __commonJSMin(((exports) => {
...
//#endregion
//#region node_modules/css-tree/cjs/tokenizer/OffsetToLocation.cjs
var require_OffsetToLocation = /* @__PURE__ */ __commonJSMin(((exports) => {
...
//#endregion
//#region node_modules/css-tree/cjs/tokenizer/TokenStream.cjs
var require_TokenStream = /* @__PURE__ */ __commonJSMin(((exports) => {
	var adoptBuffer = require_adopt_buffer();
	var utils = require_utils();
	var names = require_names$1();
	...
	var TokenStream = class {
		...
		lookupValue(offset, referenceStr) {
			offset += this.tokenIndex;
			if (offset < this.tokenCount) return utils.cmpStr(this.source, this.offsetAndType[offset - 1] & OFFSET_MASK, this.offsetAndType[offset] & OFFSET_MASK, referenceStr);
			return false;
		}
		...
		getBlockTokenPairIndex(tokenIndex) {
			...
			if (blockTokens[type] === 1) {
				...
				const pairIndex = this.balance[tokenIndex];
				return balancePair[this.getTokenType(pairIndex)] === type ? pairIndex : -1;
			}
			return -1;
		}
		...
		isDelim(code, offset) {
			if (offset) return this.lookupType(offset) === types.Delim && this.source.charCodeAt(this.lookupOffset(offset)) === code;
		...
		skipSC() {
			while (this.tokenType === types.WhiteSpace || this.tokenType === types.Comment) this.next();
		}
...
//#endregion
//#region node_modules/css-tree/cjs/tokenizer/index.cjs
			...
			for (; offset < source.length; offset++) {
				...
				switch (charCodeDefinitions.charCodeCategory(code)) {
					...
					case 92:
						...
						if (charCodeDefinitions.isNewline(nextCode)) offset += utils.getNewlineLength(source, offset + 1, nextCode);
						else if (charCodeDefinitions.isValidEscape(code, nextCode)) offset = utils.consumeEscaped(source, offset) - 1;
						break;
				}
			}
		}
		...
		while (offset < sourceLength) {
			...
			switch (charCodeDefinitions.charCodeCategory(code)) {
				...
				case 41:
					...
					break;
				case 43:
					if (charCodeDefinitions.isNumberStart(code, getCharCode(offset + 1), getCharCode(offset + 2))) consumeNumericToken();
					else {
						type = types.Delim;
						offset++;
					}
				...
				case 45:
					if (charCodeDefinitions.isNumberStart(code, getCharCode(offset + 1), getCharCode(offset + 2))) consumeNumericToken();
						...
						type = types.CDC;
						offset = offset + 3;
					} else if (charCodeDefinitions.isIdentifierStart(code, getCharCode(offset + 1), getCharCode(offset + 2))) consumeIdentLikeToken();
					else {
						type = types.Delim;
						offset++;
					}
				...
				case 91:
					...
					break;
				case 92:
					if (charCodeDefinitions.isValidEscape(code, getCharCode(offset + 1))) consumeIdentLikeToken();
					else {
						type = types.Delim;
						offset++;
					}
...
//#endregion
//#region node_modules/css-tree/cjs/utils/List.cjs
...
//#endregion
//#region node_modules/css-tree/cjs/utils/create-custom-error.cjs
var require_create_custom_error = /* @__PURE__ */ __commonJSMin(((exports) => {
...
//#endregion
//#region node_modules/css-tree/cjs/parser/SyntaxError.cjs
var require_SyntaxError$1 = /* @__PURE__ */ __commonJSMin(((exports) => {
...
//#endregion
//#region node_modules/css-tree/cjs/parser/sequence.cjs
...
//#endregion
//#region node_modules/css-tree/cjs/parser/create.cjs
var require_create$4 = /* @__PURE__ */ __commonJSMin(((exports) => {
	...
	function createParser(config) {
		...
		const parser = Object.assign(new TokenStream.TokenStream(), processConfig(config || {}), {
			...
			lookupNonWSType(offset) {
				...
				do {
					...
					if (type !== types.WhiteSpace && type !== types.Comment) return type;
				} while (type !== NULL);
				return NULL;
			},
			charCodeAt(offset) {
			...
			eatIdent(name) {
				if (this.tokenType !== types.Ident || this.lookupValue(0, name) === false) this.error(`Identifier "${name}" is expected`);
				this.next();
			},
			eatDelim(code) {
			...
			error(message, offset) {
				...
				throw new SyntaxError.SyntaxError(message || "Unexpected input", source, location.offset, location.line, location.column, locationMap.startLine, locationMap.startColumn);
			}
		});
		const createTokenIterateAPI = () => ({
			...
			getTokenTypeName: (index) => names[parser.getTokenType(index)],
		...
		const parse = function(source_, options) {
			...
			if (typeof onComment === "function") parser.forEachToken((type, start, end) => {
				if (type === types.Comment) {
					...
					onComment(utils.cmpStr(source, end - 2, end, "*/") ? source.slice(start + 2, end - 2) : source.slice(start + 2, end), loc);
				}
			});
...
//#endregion
//#region node_modules/css-tree/cjs/generator/sourceMap.cjs
...
//#endregion
//#region node_modules/css-tree/cjs/generator/token-before.cjs
var require_token_before = /* @__PURE__ */ __commonJSMin(((exports) => {
	var types = require_types();
	var PLUSSIGN = 43;
	var HYPHENMINUS = 45;
	var code = (type, value) => {
		if (type === types.Delim) type = value;
	...
	function createMap(pairs) {
		...
		return function(prevCode, type, value) {
			...
			return nextCode | (nextCharCode === HYPHENMINUS && type !== types.Ident && type !== types.Function && type !== types.CDC || nextCharCode === PLUSSIGN ? isWhiteSpaceRequired.has((prevCode & 65534) << 16 | nextCharCode << 7) : isWhiteSpaceRequired.has((prevCode & 65534) << 16 | nextCode));
		};
	}
	var spec = createMap(specPairs);
	exports.safe = createMap(safePairs);
	exports.spec = spec;
}));
//#endregion
//#region node_modules/css-tree/cjs/generator/create.cjs
var require_create$3 = /* @__PURE__ */ __commonJSMin(((exports) => {
	...
	function createGenerator(config) {
		...
		for (let [name, item] of Object.entries(config.node)) if (typeof (item.generate || item) === "function") types$1.set(name, item.generate || item);
		return function(node, options) {
			let buffer = "";
			let prevCode = 0;
			let handlers = {
				node(node) {
					if (types$1.has(node.type)) types$1.get(node.type).call(publicApi, node);
					else throw new Error("Unknown node type: " + node.type);
				},
				tokenBefore: tokenBefore.safe,
				token(type, value, suppressAutoWhiteSpace) {
					prevCode = this.tokenBefore(prevCode, type, value);
					if (!suppressAutoWhiteSpace && prevCode & 1) this.emit(" ", types.WhiteSpace, true);
					this.emit(value, type, false);
					if (type === types.Delim && value.charCodeAt(0) === REVERSESOLIDUS) this.emit("\n", types.WhiteSpace, true);
				},
				emit(value) {
					buffer += value;
				},
			...
			if (options) {
				...
				if (options.sourceMap) handlers = sourceMap.generateSourceMap(handlers);
				if (options.mode in tokenBefore) handlers.tokenBefore = tokenBefore[options.mode];
			}
			const publicApi = {
...
//#endregion
//#region node_modules/css-tree/cjs/convertor/create.cjs
var require_create$2 = /* @__PURE__ */ __commonJSMin(((exports) => {
	...
	function createConvertor(walk) {
		return {
			fromPlainObject(ast) {
				walk(ast, { enter(node) {
					if (node.children && node.children instanceof List.List === false) node.children = new List.List().fromArray(node.children);
				} });
				return ast;
			},
			toPlainObject(ast) {
				walk(ast, { leave(node) {
					if (node.children && node.children instanceof List.List) node.children = node.children.toArray();
				} });
				return ast;
			}
		};
...
//#endregion
//#region node_modules/css-tree/cjs/walker/create.cjs
var require_create$1 = /* @__PURE__ */ __commonJSMin(((exports) => {
	...
	function getWalkersFromStructure(name, nodeType) {
		...
		for (const key in structure) {
			if (hasOwnProperty.call(structure, key) === false) continue;
	...
	function getTypesFromConfig(config) {
		const types = {};
		for (const name in config.node) if (hasOwnProperty.call(config.node, name)) {
	...
	function createWalker(config) {
		...
		for (const name in types) if (hasOwnProperty.call(types, name) && types[name] !== null) {
...
//#endregion
//#region node_modules/css-tree/cjs/definition-syntax/generate.cjs
...
//#endregion
//#region node_modules/css-tree/cjs/lexer/error.cjs
...
//#endregion
//#region node_modules/css-tree/cjs/utils/names.cjs
...
//#endregion
//#region node_modules/css-tree/cjs/lexer/generic-const.cjs
var require_generic_const = /* @__PURE__ */ __commonJSMin(((exports) => {
...
//#endregion
//#region node_modules/css-tree/cjs/lexer/generic-an-plus-b.cjs
var require_generic_an_plus_b = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	var charCodeDefinitions = require_char_code_definitions();
	var types = require_types();
	var utils = require_utils();
	var PLUSSIGN = 43;
	...
	function skipSC(token, offset, getNextToken) {
		while (token !== null && (token.type === types.WhiteSpace || token.type === types.Comment)) token = getNextToken(++offset);
		return offset;
	}
	function checkInteger(token, valueOffset, disallowSign, offset) {
		...
		for (; valueOffset < token.value.length; valueOffset++) if (!charCodeDefinitions.isDigit(token.value.charCodeAt(valueOffset))) return 0;
		return offset + 1;
	}
	function consumeB(token, offset_, getNextToken) {
		...
		if (token.type !== types.Number) if (isDelim(token, PLUSSIGN) || isDelim(token, HYPHENMINUS)) {
			sign = true;
			offset = skipSC(getNextToken(++offset), offset, getNextToken);
			token = getNextToken(offset);
			if (token === null || token.type !== types.Number) return 0;
		} else return offset_;
		if (!sign) {
			const code = token.value.charCodeAt(0);
			if (code !== PLUSSIGN && code !== HYPHENMINUS) return 0;
		}
	...
	function anPlusB(token, getNextToken) {
		let offset = 0;
		if (!token) return 0;
		if (token.type === types.Number) return checkInteger(token, 0, ALLOW_SIGN, offset);
		else if (token.type === types.Ident && token.value.charCodeAt(0) === HYPHENMINUS) {
			if (!utils.cmpChar(token.value, 1, N)) return 0;
			switch (token.value.length) {
				case 2: return consumeB(getNextToken(++offset), offset, getNextToken);
				case 3:
			...
			if (token.type !== types.Ident) token = getNextToken(++offset);
			if (token === null || !utils.cmpChar(token.value, 0, N)) return 0;
			switch (token.value.length) {
				case 1: return consumeB(getNextToken(++offset), offset, getNextToken);
				case 2:
			...
			let i = sign;
			for (; i < token.value.length; i++) if (!charCodeDefinitions.isDigit(token.value.charCodeAt(i))) break;
			if (i === sign) return 0;
			if (!utils.cmpChar(token.value, i, N)) return 0;
			if (i + 1 === token.value.length) return consumeB(getNextToken(++offset), offset, getNextToken);
			else {
				if (token.value.charCodeAt(i + 1) !== HYPHENMINUS) return 0;
				if (i + 2 === token.value.length) {
					offset = skipSC(getNextToken(++offset), offset, getNextToken);
					token = getNextToken(offset);
					return checkInteger(token, 0, DISALLOW_SIGN, offset);
				} else return checkInteger(token, i + 2, DISALLOW_SIGN, offset);
			}
		}
		return 0;
	}
...
//#endregion
//#region node_modules/css-tree/cjs/lexer/generic-urange.cjs
var require_generic_urange = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	var charCodeDefinitions = require_char_code_definitions();
	var types = require_types();
	var utils = require_utils();
	var PLUSSIGN = 43;
	...
	function hexSequence(token, offset, allowDash) {
		...
		for (let pos = offset; pos < token.value.length; pos++) {
			...
			if (!charCodeDefinitions.isHexDigit(code)) return 0;
	...
	function urange(token, getNextToken) {
		let length = 0;
		if (token === null || token.type !== types.Ident || !utils.cmpChar(token.value, 0, U)) return 0;
		token = getNextToken(++length);
		if (token === null) return 0;
		if (isDelim(token, PLUSSIGN)) {
			token = getNextToken(++length);
			if (token === null) return 0;
			if (token.type === types.Ident) return withQuestionMarkSequence(hexSequence(token, 0, true), ++length, getNextToken);
			if (isDelim(token, QUESTIONMARK)) return withQuestionMarkSequence(1, ++length, getNextToken);
			return 0;
		}
		if (token.type === types.Number) {
			...
			if (token.type === types.Dimension || token.type === types.Number) {
				if (!startsWith(token, HYPHENMINUS) || !hexSequence(token, 1, false)) return 0;
				return length + 1;
			}
			return withQuestionMarkSequence(consumedHexLength, length, getNextToken);
		}
		if (token.type === types.Dimension) return withQuestionMarkSequence(hexSequence(token, 1, true), ++length, getNextToken);
		return 0;
	}
	module.exports = urange;
}));
//#endregion
//#region node_modules/css-tree/cjs/lexer/generic.cjs
var require_generic = /* @__PURE__ */ __commonJSMin(((exports) => {
	...
	function math(next, functionNames) {
		return function(token, getNextToken, opts) {
			if (token === null) return 0;
			if (token.type === types.Function && eqStrAny(token.value, functionNames)) return consumeFunction(token, getNextToken);
	...
	function customIdent(token) {
		if (token === null || token.type !== types.Ident) return 0;
		const name = token.value.toLowerCase();
		if (eqStrAny(name, genericConst.cssWideKeywords)) return 0;
	...
	function hexColor(token) {
		if (token === null || token.type !== types.Hash) return 0;
	...
	function dimension(type) {
		...
		return function(token, getNextToken, opts) {
			if (token === null || token.type !== types.Dimension) return 0;
	...
	function percentage(token, getNextToken, opts) {
		if (token === null || token.type !== types.Percentage) return 0;
	...
	function integer(token, getNextToken, opts) {
		...
		for (; i < token.value.length; i++) if (!charCodeDefinitions.isDigit(charCodeAt(token.value, i))) return 0;
	...
	function createAttrUnit(units) {
		...
		return function attrUnit(token) {
			if (token === null) return 0;
			if (token.type === types.Delim && token.value === "%") return 1;
			if (token.type === types.Ident && unitSet.has(token.value.toLowerCase())) return 1;
			return 0;
		};
	}
...
//#endregion
//#region node_modules/css-tree/cjs/lexer/units.cjs
...
//#endregion
//#region node_modules/css-tree/cjs/lexer/prepare-tokens.cjs
var require_prepare_tokens = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	var index = require_tokenizer();
...
//#endregion
//#region node_modules/css-tree/cjs/definition-syntax/SyntaxError.cjs
...
//#endregion
//#region node_modules/css-tree/cjs/definition-syntax/scanner.cjs
	...
	var Scanner = class {
		...
		findWsEnd(pos) {
			for (; pos < this.str.length; pos++) {
				...
				if (code !== R && code !== N && code !== F && code !== SPACE && code !== TAB) break;
			}
			return pos;
		}
		substringToPos(end) {
...
//#endregion
//#region node_modules/css-tree/cjs/definition-syntax/parse.cjs
var require_parse = /* @__PURE__ */ __commonJSMin(((exports) => {
	...
	function maybeMultiplied(scanner, node) {
		...
		if (multiplier !== null) {
			...
			if (scanner.charCode() === NUMBERSIGN && scanner.charCodeAt(scanner.pos - 1) === PLUSSIGN) return maybeMultiplied(scanner, multiplier);
			if (scanner.charCode() === QUESTIONMARK && scanner.charCodeAt(scanner.pos - 1) === RIGHTCURLYBRACKET) return maybeMultiplied(scanner, multiplier);
			return multiplier;
		}
		return node;
	}
	...
	function peek(scanner) {
		...
		switch (code) {
			...
			case LESSTHANSIGN: return scanner.nextCharCode() === APOSTROPHE ? readProperty(scanner) : readType(scanner);
			...
			case COMMA:
				scanner.pos++;
				return { type: "Comma" };
			case APOSTROPHE: return maybeMultiplied(scanner, {
				type: "String",
	...
	function parse(source) {
		...
		if (scanner$1.pos !== source.length) scanner$1.error("Unexpected input");
...
//#endregion
//#region node_modules/css-tree/cjs/lexer/match-graph.cjs
var require_match_graph = /* @__PURE__ */ __commonJSMin(((exports) => {
	...
	function buildMatchGraph(syntaxTree, ref) {
		if (typeof syntaxTree === "string") syntaxTree = parse.parse(syntaxTree);
		return {
			type: "MatchGraph",
...
//#endregion
//#region node_modules/css-tree/cjs/lexer/match.cjs
var require_match = /* @__PURE__ */ __commonJSMin(((exports) => {
	...
	function isContextEdgeDelim(token) {
		if (token.type !== types.Delim) return false;
		return token.value !== "?";
	}
	...
	function internalMatch(tokens, state, syntaxes) {
		...
		while (exitReason === null && ++iterationCount < ITERATION_LIMIT) switch (state.type) {
			case "Match":
				...
				if (state === matchGraph.DISALLOW_EMPTY) if (thenStack.matchStack === matchStack) {
					state = matchGraph.MISMATCH;
					break;
				} else state = matchGraph.MATCH;
			...
			case "MatchGraph":
				...
				break;
			case "If":
				if (state.else !== matchGraph.MISMATCH) pushElseStack(state.else);
				if (state.then !== matchGraph.MATCH) pushThenStack(state.then);
				state = state.match;
				break;
			case "MatchOnce":
				state = {
			...
			case "Generic": {
				...
				if (!isNaN(lastTokenIndex) && lastTokenIndex > tokenIndex) {
					...
					state = matchGraph.MATCH;
				} else state = matchGraph.MISMATCH;
				break;
			}
			case "Type":
			case "Property": {
				...
				if (syntaxStash !== false && token !== null && state.type === "Type") {
					if (state.name === "custom-ident" && token.type === types.Ident || state.name === "length" && token.value === "0") {
			...
			case "Token":
				...
				break;
			case "Comma":
				if (token !== null && token.type === types.Comma) if (isCommaContextStart(matchStack.token)) state = matchGraph.MISMATCH;
				else {
					addTokenToMatch();
					state = isCommaContextEnd(token) ? matchGraph.MISMATCH : matchGraph.MATCH;
				}
				else state = isCommaContextStart(matchStack.token) || isCommaContextEnd(token) ? matchGraph.MATCH : matchGraph.MISMATCH;
				break;
			case "String":
				let string = "";
...
//#endregion
//#region node_modules/css-tree/cjs/lexer/trace.cjs
...
//#endregion
//#region node_modules/css-tree/cjs/lexer/search.cjs
...
//#endregion
//#region node_modules/css-tree/cjs/lexer/structure.cjs
var require_structure = /* @__PURE__ */ __commonJSMin(((exports) => {
	var List = require_List();
	var { hasOwnProperty } = Object.prototype;
	...
	function createNodeStructureChecker(type, fields) {
		return function checkNode(node, warn) {
			...
			for (let key in node) {
				let valid = true;
				if (hasOwnProperty.call(node, key) === false) continue;
				if (key === "type") {
					...
					for (let i = 0; !valid && i < fields[key].length; i++) {
						...
						switch (fieldType) {
							...
							else if (Array.isArray(fieldType)) valid = node[key] instanceof List.List;
						}
					}
				} else warn(node, "Unknown field `" + key + "` for " + type + " node type");
				if (!valid) warn(node, "Bad value for `" + type + "." + key + "`");
			}
			for (const key in fields) if (hasOwnProperty.call(fields, key) && hasOwnProperty.call(node, key) === false) warn(node, "Field `" + type + "." + key + "` is missed");
		};
	}
...
//#endregion
//#region node_modules/css-tree/cjs/definition-syntax/walk.cjs
...
//#endregion
//#region node_modules/css-tree/cjs/lexer/Lexer.cjs
var require_Lexer = /* @__PURE__ */ __commonJSMin(((exports) => {
	...
	function dumpMapSyntax(map, compact, syntaxAsAst) {
		const result = {};
		for (const name in map) if (map[name].syntax) result[name] = syntaxAsAst ? map[name].syntax : generate.generate(map[name].syntax, { compact });
		return result;
	}
	...
	function matchSyntax(lexer, syntax, value, useCssWideKeywords) {
		...
		if (useCssWideKeywords) result = match.matchAsTree(tokens, lexer.cssWideKeywordsSyntax, lexer);
		if (!useCssWideKeywords || !result.match) {
			result = match.matchAsTree(tokens, syntax.match, lexer);
			if (!result.match) return buildMatchResult(null, new error.SyntaxMatchError(result.reason, syntax.syntax, value, result), result.iterations);
		}
	...
	var Lexer = class {
		constructor(config, syntax, structure$1) {
			...
			if (config) {
				...
				if (config.units) {
					for (const group of Object.keys(units)) if (Array.isArray(config.units[group])) this.units[group] = config.units[group];
				}
				if (config.types) for (const [name, type] of Object.entries(config.types)) this.addType_(name, type);
				if (config.generic) {
					this.generic = true;
					for (const [name, value] of Object.entries(generic.createGenericTypes(this.units))) this.addType_(name, value);
				}
		...
		createDescriptor(syntax, type, name, parent = null) {
			...
			if (typeof syntax === "function") descriptor.match = matchGraph.buildMatchGraph(syntax, ref);
			else {
				if (typeof syntax === "string") Object.defineProperty(descriptor, "syntax", { get() {
					Object.defineProperty(descriptor, "syntax", { value: parse.parse(syntax) });
				...
				if (type === "Property") Object.defineProperty(descriptor, "matchRef", { get() {
					...
					const value = syntaxHasTopLevelCommaMultiplier(syntax) ? matchGraph.buildMatchGraph({
						...syntax,
		...
		checkAtruleName(atruleName) {
			if (!this.getAtrule(atruleName)) return new error.SyntaxReferenceError("Unknown at-rule", "@" + atruleName);
		}
		...
		checkAtruleDescriptorName(atruleName, descriptorName) {
			...
			if (!atrule.descriptors[descriptor.name] && !atrule.descriptors[descriptor.basename]) return new error.SyntaxReferenceError("Unknown at-rule descriptor", descriptorName);
		}
		checkPropertyName(propertyName) {
			if (!this.getProperty(propertyName)) return new error.SyntaxReferenceError("Unknown property", propertyName);
		}
		...
		matchProperty(propertyName, value) {
			if (names.property(propertyName).custom) return buildMatchResult(null, /* @__PURE__ */ new Error("Lexer matching doesn't applicable for custom properties"));
		...
		matchType(typeName, value) {
			...
			if (!typeSyntax) return buildMatchResult(null, new error.SyntaxReferenceError("Unknown type", typeName));
			return matchSyntax(this, typeSyntax, value, false);
		}
		match(syntax, value) {
			if (typeof syntax !== "string" && (!syntax || !syntax.type)) return buildMatchResult(null, new error.SyntaxReferenceError("Bad syntax"));
		...
		validate() {
			...
			function validate(syntax, name, broken, descriptor) {
				...
				if (descriptor.syntax !== null) walk.walk(descriptor.syntax, function(node) {
...
//#endregion
//#region node_modules/css-tree/cjs/syntax/config/mix.cjs
var require_mix = /* @__PURE__ */ __commonJSMin(((exports, module) => {
...
//#endregion
//#region node_modules/css-tree/cjs/syntax/create.cjs
var require_create = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	var index = require_tokenizer();
	var create = require_create$4();
	var create$2 = require_create$3();
	var create$3 = require_create$2();
	var create$1 = require_create$1();
...
//#endregion
//#region node_modules/css-tree/data/patch.json
var patch_exports = /* @__PURE__ */ __exportAll({
	atrules: () => atrules,
	default: () => patch_default,
	properties: () => properties,
	types: () => types
});
var atrules, properties, types, patch_default;
var init_patch = __esmMin((() => {
	atrules = {
...
//#endregion
//#region node_modules/css-tree/cjs/data-patch.cjs
var require_data_patch = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	module.exports = (init_patch(), __toCommonJS(patch_exports).default);
}));
//#endregion
//#region node_modules/mdn-data/css/at-rules.json
var at_rules_exports = /* @__PURE__ */ __exportAll({ default: () => at_rules_default });
var at_rules_default;
var init_at_rules = __esmMin((() => {
	at_rules_default = {
		"@charset": {
		...
		"@font-palette-values": {
			...
			"interfaces": ["CSSFontPaletteValuesRule"],
			"groups": ["CSS Fonts"],
			"descriptors": {
				"base-palette": {
		...
		"@view-transition": {
			...
			"interfaces": ["CSSViewTransitionRule"],
...
//#endregion
//#region node_modules/mdn-data/css/properties.json
...
var all, animation, appearance, background, border, bottom, caret, clear, clip, color$1, columns, contain, container, content, cursor, cx, cy, d, direction, display, fill, filter, flex, float, font, gap, grid, height, hyphens, inset, isolation, interactivity, left, margin, marker, mask, offset, opacity, order, orphans, outline, overflow, overlay, padding, page, perspective, position$1, quotes, r, resize, right, rotate, rx, ry, scale, stroke, top, transform, transition, translate, visibility, widows, width, x, y, zoom, properties_default;
var init_properties = __esmMin((() => {
	...
	bottom = {
		...
		"groups": ["CSS Anchor Positioning", "CSS Positioned Layout"],
		"initial": "auto",
	...
	columns = {
		...
		"percentages": "no",
		"groups": ["CSS Multi-column Layout"],
		"initial": [
			"column-width",
			"column-count",
			"column-height"
	...
	inset = {
		...
		"groups": ["CSS Logical Properties and Values", "CSS Positioned Layout"],
		"initial": [
			"top",
			"bottom",
			"left",
	...
	properties_default = {
		"--*": {
			...
			"animationType": "discrete",
			"percentages": "no",
			"groups": ["CSS Custom Properties for Cascading Variables"],
			"initial": "seeProse",
			"appliesto": "allElements",
		...
		"align-content": {
			...
			"animationType": "discrete",
			"percentages": "no",
			"groups": ["CSS Box Alignment", "CSS Flexible Box Layout"],
			"initial": "normal",
		...
		"animation-range": {
			...
			"groups": ["Scroll-driven Animations"],
		...
		"block-size": {
			...
			"groups": ["CSS Logical Properties and Values"],
			"initial": "auto",
		...
		"column-gap": {
			...
			"animationType": "lpc",
			"percentages": "referToDimensionOfContentArea",
			"groups": ["CSS Box Alignment", "CSS Multi-column Layout"],
			"initial": "normal",
			"appliesto": "multiColumnElementsFlexContainersGridContainers",
		...
		"column-height": {
			...
			"animationType": "byComputedValueType",
			"percentages": "no",
			"groups": ["CSS Box Sizing", "CSS Multi-column Layout"],
			"initial": "auto",
			"appliesto": "blockContainersExceptTableWrappers",
		...
		"corner-block-end-shape": {
			...
			"media": "visual",
			"inherited": false,
			"animationType": ["corner-end-start-shape", "corner-end-end-shape"],
			"percentages": "no",
			"groups": ["CSS Backgrounds and Borders"],
			"initial": ["corner-end-start-shape", "corner-end-end-shape"],
			"appliesto": "allElements",
			"computed": ["corner-end-start-shape", "corner-end-end-shape"],
			"order": "perGrammar",
			"alsoAppliesTo": ["::first-letter"],
			"status": "standard",
		...
		"corner-block-start-shape": {
			...
			"media": "visual",
			"inherited": false,
			"animationType": ["corner-start-start-shape", "corner-start-end-shape"],
			"percentages": "no",
			"groups": ["CSS Backgrounds and Borders"],
			"initial": ["corner-start-start-shape", "corner-start-end-shape"],
			"appliesto": "allElements",
			"computed": ["corner-start-start-shape", "corner-start-end-shape"],
			"order": "perGrammar",
			"alsoAppliesTo": ["::first-letter"],
			"status": "standard",
		...
		"corner-bottom-shape": {
			...
			"media": "visual",
			"inherited": false,
			"animationType": ["corner-bottom-left-shape", "corner-bottom-right-shape"],
			"percentages": "no",
			"groups": ["CSS Backgrounds and Borders"],
			"initial": ["corner-bottom-left-shape", "corner-bottom-right-shape"],
			"appliesto": "allElements",
			"computed": ["corner-bottom-left-shape", "corner-bottom-right-shape"],
			"order": "perGrammar",
			"alsoAppliesTo": ["::first-letter"],
			"status": "standard",
		...
		"corner-inline-end-shape": {
			...
			"media": "visual",
			"inherited": false,
			"animationType": ["corner-start-end-shape", "corner-end-end-shape"],
			"percentages": "no",
			"groups": ["CSS Backgrounds and Borders"],
			"initial": ["corner-start-end-shape", "corner-end-end-shape"],
			"appliesto": "allElements",
			"computed": ["corner-start-end-shape", "corner-end-end-shape"],
			"order": "perGrammar",
			"alsoAppliesTo": ["::first-letter"],
			"status": "standard",
		...
		"corner-left-shape": {
			...
			"media": "visual",
			"inherited": false,
			"animationType": ["corner-top-left-shape", "corner-bottom-left-shape"],
			"percentages": "no",
			"groups": ["CSS Backgrounds and Borders"],
			"initial": ["corner-top-left-shape", "corner-bottom-left-shape"],
			"appliesto": "allElements",
			"computed": ["corner-top-left-shape", "corner-bottom-left-shape"],
			"order": "perGrammar",
			"alsoAppliesTo": ["::first-letter"],
			"status": "standard",
		...
		"corner-right-shape": {
			...
			"media": "visual",
			"inherited": false,
			"animationType": ["corner-top-right-shape", "corner-bottom-right-shape"],
			"percentages": "no",
			"groups": ["CSS Backgrounds and Borders"],
			"initial": ["corner-top-right-shape", "corner-bottom-right-shape"],
			"appliesto": "allElements",
			"computed": ["corner-top-right-shape", "corner-bottom-right-shape"],
			"order": "perGrammar",
			"alsoAppliesTo": ["::first-letter"],
			"status": "standard",
		...
		"corner-top-shape": {
			...
			"media": "visual",
			"inherited": false,
			"animationType": ["corner-top-left-shape", "corner-top-right-shape"],
			"percentages": "no",
			"groups": ["CSS Backgrounds and Borders"],
			"initial": ["corner-top-left-shape", "corner-top-right-shape"],
			"appliesto": "allElements",
			"computed": ["corner-top-left-shape", "corner-top-right-shape"],
			"order": "perGrammar",
			"alsoAppliesTo": ["::first-letter"],
			"status": "standard",
		...
		"dominant-baseline": {
			...
			"animationType": "discrete",
			"percentages": "no",
			"groups": ["CSS Inline", "Scalable Vector Graphics"],
			"initial": "auto",
		...
		"interest-delay": {
			...
			"media": "visual",
			"inherited": true,
			"animationType": ["interest-delay-start", "interest-delay-end"],
			"percentages": "no",
			"groups": ["CSS Basic User Interface"],
			"initial": ["interest-delay-start", "interest-delay-end"],
			"appliesto": "allElements",
			"computed": ["interest-delay-start", "interest-delay-end"],
			"order": "perGrammar",
			"status": "standard",
		...
		"margin-bottom": {
			...
			"animationType": "length",
			"percentages": "referToWidthOfContainingBlock",
			"groups": ["CSS Anchor Positioning", "CSS Box Model"],
			"initial": "0",
			"appliesto": "allElementsExceptTableDisplayTypes",
			"computed": "percentageAsSpecifiedOrAbsoluteLength",
			"order": "uniqueOrder",
		...
		"overscroll-behavior": {
			...
			"animationType": "discrete",
			"percentages": "no",
			"groups": ["CSS Overscroll Behavior"],
			"initial": "auto",
			"appliesto": "nonReplacedBlockAndInlineBlockElements",
		...
		"text-size-adjust": {
			...
			"groups": ["CSS Mobile Text Size Adjustment"],
		...
		"text-wrap": {
			...
			"percentages": ["text-wrap-mode", "text-wrap-style"],
			"groups": ["CSS Text"],
			"initial": "wrap",
		...
		"timeline-trigger-exit-range": {
			...
			"inherited": false,
			"animationType": "notAnimatable",
			"percentages": ["timeline-trigger-exit-range-start", "timeline-trigger-exit-range-end"],
			"groups": ["CSS Animations"],
			"initial": ["timeline-trigger-exit-range-start", "timeline-trigger-exit-range-end"],
			"appliesto": "allElements",
			"computed": ["timeline-trigger-exit-range-start", "timeline-trigger-exit-range-end"],
			"order": "perGrammar",
			"status": "standard",
		...
		"timeline-trigger-range": {
			...
			"inherited": false,
			"animationType": "notAnimatable",
			"percentages": ["timeline-trigger-range-start", "timeline-trigger-range-end"],
			"groups": ["CSS Animations"],
			"initial": ["timeline-trigger-range-start", "timeline-trigger-range-end"],
			"appliesto": "allElements",
			"computed": ["timeline-trigger-range-start", "timeline-trigger-range-end"],
			"order": "perGrammar",
			"status": "standard",
...
//#endregion
//#region node_modules/mdn-data/css/syntaxes.json
var syntaxes_exports = /* @__PURE__ */ __exportAll({
...
var attachment, axis, color, combinator, counter, dasharray, gradient, hue, image, integer, paint, position, quote, ratio, scroller, shadow, shape, size, symbol, target, xyz, syntaxes_default;
var init_syntaxes = __esmMin((() => {
	...
	integer = { "syntax": "<number-token>" };
	paint = { "syntax": "none | <color> | <url> [none | <color>]? | context-fill | context-stroke" };
	...
	xyz = { "syntax": "xyz | xyz-d50 | xyz-d65" };
	syntaxes_default = {
		...
		"an+b": { "syntax": "odd | even | <integer> | <n-dimension> | '+'?† n | -n | <ndashdigit-dimension> | '+'?† <ndashdigit-ident> | <dashndashdigit-ident> | <n-dimension> <signed-integer> | '+'?† n <signed-integer> | -n <signed-integer> | <ndash-dimension> <signless-integer> | '+'?† n- <signless-integer> | -n- <signless-integer> | <n-dimension> ['+' | '-'] <signless-integer> | '+'?† n ['+' | '-'] <signless-integer> | -n ['+' | '-'] <signless-integer>" },
		"anchor()": { "syntax": "anchor( <anchor-name>? && <anchor-side>, <length-percentage>? )" },
		"anchor-name": { "syntax": "<dashed-ident>" },
		...
		"anchor-size()": { "syntax": "anchor-size( [ <anchor-name> || <anchor-size> ]? , <length-percentage>? )" },
		"angle-percentage": { "syntax": "<angle> | <percentage>" },
		"angular-color-hint": { "syntax": "<angle-percentage> | <zero>" },
		...
		"animation-action": { "syntax": "none | play | play-once | play-forwards | play-backwards | pause | reset | replay" },
		"asin()": { "syntax": "asin( <calc-sum> )" },
		"atan()": { "syntax": "atan( <calc-sum> )" },
		"atan2()": { "syntax": "atan2( <calc-sum>, <calc-sum> )" },
		attachment,
		"attr()": { "syntax": "attr( <attr-name> <attr-type>? , <declaration-value>? )" },
		"attr-matcher": { "syntax": "[ '~' | '|' | '^' | '$' | '*' ]? '='" },
		"attr-modifier": { "syntax": "i | s" },
		"attr-type": { "syntax": "type( <syntax> ) | raw-string | number | <attr-unit>" },
		...
		"basic-shape-rect": { "syntax": "<inset()> | <rect()> | <xywh()>" },
		"bg-clip": { "syntax": "<visual-box> | border-area | text" },
		"bg-image": { "syntax": "<image> | none" },
		"bg-layer": { "syntax": "<bg-image> || <bg-position> [ / <bg-size> ]? || <repeat-style> || <attachment> || <visual-box> || <visual-box>" },
		...
		"bg-size": { "syntax": "[ <length-percentage [0,∞]> | auto ]{1,2} | cover | contain" },
		...
		"calc-size()": { "syntax": "calc-size( <calc-size-basis>, <calc-sum> )" },
		"calc-size-basis": { "syntax": "<intrinsic-size-keyword> | <calc-size()> | any | <calc-sum>" },
		...
		"circle()": { "syntax": "circle( <radial-size>? [ at <position> ]? )" },
		"clamp()": { "syntax": "clamp( <calc-sum>#{3} )" },
		"class-selector": { "syntax": "'.' <ident-token>" },
		"clip-source": { "syntax": "<url>" },
		color,
		"color()": { "syntax": "color( [ from <color> ]? <colorspace-params> [ / [ <alpha-value> | none ] ]? )" },
		...
		"color-stop-angle": { "syntax": "[ <angle-percentage> | <zero> ]{1,2}" },
		...
		"colorspace-params": { "syntax": "[<custom-params> | <predefined-rgb-params> | <xyz-params>]" },
		combinator,
		"common-lig-values": { "syntax": "[ common-ligatures | no-common-ligatures ]" },
		"compat-auto": { "syntax": "searchfield | textarea | checkbox | radio | menulist | listbox | meter | progress-bar | button" },
		"compat-special": { "syntax": "textfield | menulist-button" },
		...
		"conic-gradient()": { "syntax": "conic-gradient( [ <conic-gradient-syntax> ] )" },
		"conic-gradient-syntax": { "syntax": "[ [ [ from [ <angle> | <zero> ] ]? [ at <position> ]? ] || <color-interpolation-method> ]? , <angular-color-stop-list>" },
		"container-condition": { "syntax": "[ <container-name>? <container-query>? ]!" },
		"container-name": { "syntax": "<custom-ident>" },
		"container-query": { "syntax": "not <query-in-parens> | <query-in-parens> [ [ and <query-in-parens> ]* | [ or <query-in-parens> ]* ]" },
		...
		"coord-box": { "syntax": "<paint-box> | view-box" },
		"corner-shape-value": { "syntax": "round | scoop | bevel | notch | square | squircle | <superellipse()>" },
		...
		"cubic-bezier()": { "syntax": "cubic-bezier( [ <number [0,1]>, <number> ]#{2} )" },
		"cubic-bezier-easing-function": { "syntax": "ease | ease-in | ease-out | ease-in-out | <cubic-bezier()>" },
		"cursor-predefined": { "syntax": "auto | default | none | context-menu | help | pointer | progress | wait | cell | crosshair | text | vertical-text | alias | copy | move | no-drop | not-allowed | e-resize | n-resize | ne-resize | nw-resize | s-resize | se-resize | sw-resize | w-resize | ew-resize | ns-resize | nesw-resize | nwse-resize | col-resize | row-resize | all-scroll | zoom-in | zoom-out | grab | grabbing" },
		"custom-color-space": { "syntax": "<dashed-ident>" },
		"custom-params": { "syntax": "<dashed-ident> [ <number> | <percentage> | none ]+" },
		dasharray,
		"dashndashdigit-ident": { "syntax": "<ident-token>" },
		...
		"drop-shadow()": { "syntax": "drop-shadow( [ <color>? && <length>{2,3} ] )" },
		"dynamic-range-limit-mix()": { "syntax": "dynamic-range-limit-mix( [ <'dynamic-range-limit'> && <percentage [0,100]> ]#{2,} )" },
		"easing-function": { "syntax": "<linear-easing-function> | <cubic-bezier-easing-function> | <step-easing-function>" },
		...
		"ellipse()": { "syntax": "ellipse( <radial-size>? [ at <position> ]? )" },
		...
		"filter-value-list": { "syntax": "[ <filter-function> | <url> ]+" },
		"final-bg-layer": { "syntax": "<bg-image> || <bg-position> [ / <bg-size> ]? || <repeat-style> || <attachment> || <visual-box> || <visual-box> || <'background-color'>" },
		"fit-content()": { "syntax": "fit-content( <length-percentage [0,∞]> )" },
		...
		"font-variant-css2": { "syntax": "normal | small-caps" },
		"font-weight-absolute": { "syntax": "normal | bold | <number [1,1000]>" },
		"font-width-css3": { "syntax": "normal | ultra-condensed | extra-condensed | condensed | semi-condensed | semi-expanded | expanded | extra-expanded | ultra-expanded" },
		"form-control-identifier": { "syntax": "select" },
		...
		"generic-family": { "syntax": "<generic-complete> | <generic-incomplete> | emoji | fangsong" },
		...
		"hsl()": { "syntax": "hsl( <hue>, <percentage>, <percentage>, <alpha-value>? ) | hsl( [ <hue> | none ] [ <percentage> | <number> | none ] [ <percentage> | <number> | none ] [ / [ <alpha-value> | none ] ]? )" },
		"hsla()": { "syntax": "hsla( <hue>, <percentage>, <percentage>, <alpha-value>? ) | hsla( [ <hue> | none ] [ <percentage> | <number> | none ] [ <percentage> | <number> | none ] [ / [ <alpha-value> | none ] ]? )" },
		hue,
		"hue-interpolation-method": { "syntax": "[ shorter | longer | increasing | decreasing ] hue" },
		"hue-rotate()": { "syntax": "hue-rotate( [ <angle> | <zero> ]? )" },
		"hwb()": { "syntax": "hwb( [ <hue> | none ] [ <percentage> | <number> | none ] [ <percentage> | <number> | none ] [ / [ <alpha-value> | none ] ]? )" },
		...
		"keyframe-selector": { "syntax": "from | to | <percentage [0,100]> | <timeline-range-name> <percentage>" },
		...
		"linear()": { "syntax": "linear( [ <number> && <percentage>{0,2} ]# )" },
		"linear-color-hint": { "syntax": "<length-percentage>" },
		"linear-color-stop": { "syntax": "<color> <color-stop-length>?" },
		"linear-easing-function": { "syntax": "linear | <linear()>" },
		"linear-gradient()": { "syntax": "linear-gradient( [ <linear-gradient-syntax> ] )" },
		"linear-gradient-syntax": { "syntax": "[ [ <angle> | <zero> | to <side-or-corner> ] || <color-interpolation-method> ]? , <color-stop-list>" },
		...
		"oklab()": { "syntax": "oklab( [ <percentage> | <number> | none] [ <percentage> | <number> | none] [ <percentage> | <number> | none] [ / [<alpha-value> | none] ]? )" },
		"oklch()": { "syntax": "oklch( [ <percentage> | <number> | none] [ <percentage> | <number> | none] [ <hue> | none] [ / [<alpha-value> | none] ]? )" },
		"opacity()": { "syntax": "opacity( [ <number> | <percentage> ]? )" },
		"opacity-value": { "syntax": "<number> | <percentage>" },
		"outline-line-style": { "syntax": "none | dotted | dashed | solid | double | groove | ridge | inset | outset" },
		...
		"paint-box": { "syntax": "<visual-box> | fill-box | stroke-box" },
		"palette-identifier": { "syntax": "<dashed-ident>" },
		"palette-mix()": { "syntax": "palette-mix(<color-interpolation-method> , [ [normal | light | dark | <palette-identifier> | <palette-mix()> ] && <percentage [0,100]>? ]#{2})" },
		...
		"predefined-rgb": { "syntax": "srgb | srgb-linear | display-p3 | display-p3-linear | a98-rgb | prophoto-rgb | rec2020" },
		...
		"query-in-parens": { "syntax": "( <container-query> ) | ( <size-feature> ) | style( <style-query> ) | scroll-state( <scroll-state-query> ) | <general-enclosed>" },
		quote,
		"radial-extent": { "syntax": "closest-corner | closest-side | farthest-corner | farthest-side" },
		"radial-gradient()": { "syntax": "radial-gradient( [ <radial-gradient-syntax> ] )" },
		"radial-gradient-syntax": { "syntax": "[ [ [ <radial-shape> || <radial-size> ]? [ at <position> ]? ] || <color-interpolation-method> ]? , <color-stop-list>" },
		"radial-shape": { "syntax": "circle | ellipse" },
		"radial-size": { "syntax": "<radial-extent> | <length [0,∞]> | <length-percentage [0,∞]>{2}" },
		ratio,
		...
		"rect()": { "syntax": "rect( [ <length-percentage> | auto ]{4} [ round <'border-radius'> ]? )" },
		"rectangular-color-space": { "syntax": "srgb | srgb-linear | display-p3 | display-p3-linear | a98-rgb | prophoto-rgb | rec2020 | lab | oklab | xyz | xyz-d50 | xyz-d65" },
		...
		"repeating-conic-gradient()": { "syntax": "repeating-conic-gradient( [ <conic-gradient-syntax> ] )" },
		"repeating-linear-gradient()": { "syntax": "repeating-linear-gradient( [ <linear-gradient-syntax> ] )" },
		"repeating-radial-gradient()": { "syntax": "repeating-radial-gradient( [ <radial-gradient-syntax> ] )" },
		"reversed-counter-name": { "syntax": "reversed( <counter-name> )" },
		"rgb()": { "syntax": "rgb( <percentage>#{3} , <alpha-value>? ) | rgb( <number>#{3} , <alpha-value>? ) | rgb( [ <number> | <percentage> | none ]{3} [ / [ <alpha-value> | none ] ]? )" },
		"rgba()": { "syntax": "rgba( <percentage>#{3} , <alpha-value>? ) | rgba( <number>#{3} , <alpha-value>? ) | rgba( [ <number> | <percentage> | none ]{3} [ / [ <alpha-value> | none ] ]? )" },
		...
		"scope-end": { "syntax": "<selector-list>" },
		"scope-start": { "syntax": "<selector-list>" },
		"scroll()": { "syntax": "scroll( [ <scroller> || <axis> ]? )" },
		scroller,
		"scroll-state-feature": { "syntax": "<media-query-list>" },
		"scroll-state-in-parens": { "syntax": "( <scroll-state-query> ) | ( <scroll-state-feature> ) | <general-enclosed>" },
		"scroll-state-query": { "syntax": "not <scroll-state-in-parens> | <scroll-state-in-parens> [ [ and <scroll-state-in-parens> ]* | [ or <scroll-state-in-parens> ]* ] | <scroll-state-feature>" },
		...
		"signed-integer": { "syntax": "<number-token>" },
		"signless-integer": { "syntax": "<number-token>" },
		...
		"single-animation-composition": { "syntax": "replace | add | accumulate" },
		...
		size,
		"size-feature": { "syntax": "<media-query-list>" },
		...
		"step-easing-function": { "syntax": "step-start | step-end | <steps()>" },
		"steps()": { "syntax": "steps( <integer>, <step-position>? )" },
		"style-feature": { "syntax": "<declaration>" },
		"style-in-parens": { "syntax": "( <style-query> ) | ( <style-feature> ) | <general-enclosed>" },
		"style-query": { "syntax": "not <style-in-parens> | <style-in-parens> [ [ and <style-in-parens> ]* | [ or <style-in-parens> ]* ] | <style-feature>" },
		"subclass-selector": { "syntax": "<id-selector> | <class-selector> | <attribute-selector> | <pseudo-class-selector>" },
		"superellipse()": { "syntax": "superellipse( [ <number> | infinity | -infinity ] )" },
		...
		symbol,
		"symbols()": { "syntax": "symbols( <symbols-type>? [ <string> | <image> ]+ )" },
		"symbols-type": { "syntax": "cyclic | numeric | alphabetic | symbolic | fixed" },
		...
		"system-family-name": { "syntax": "caption | icon | menu | message-box | small-caption | status-bar" },
		...
		"text-edge": { "syntax": "[ text | cap | ex | ideographic | ideographic-ink ] [ text | alphabetic | ideographic | ideographic-ink ]?" },
		...
		"try-size": { "syntax": "most-width | most-height | most-block-size | most-inline-size" },
		"try-tactic": { "syntax": "flip-block || flip-inline || flip-start" },
		...
		"xywh()": { "syntax": "xywh( <length-percentage>{2} <length-percentage [0,∞]>{2} [ round <'border-radius'> ]? )" },
		xyz,
		"xyz-params": { "syntax": "<xyz> [ <number> | <percentage> | none ]{3}" }
	};
}));
//#endregion
//#region node_modules/css-tree/cjs/data.cjs
var require_data = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	var dataPatch = require_data_patch();
	var mdnAtrules = (init_at_rules(), __toCommonJS(at_rules_exports).default);
	var mdnProperties = (init_properties(), __toCommonJS(properties_exports).default);
	var mdnSyntaxes = (init_syntaxes(), __toCommonJS(syntaxes_exports).default);
...
//#endregion
//#region node_modules/css-tree/cjs/syntax/node/AnPlusB.cjs
var require_AnPlusB = /* @__PURE__ */ __commonJSMin(((exports) => {
	var types = require_types();
	var charCodeDefinitions = require_char_code_definitions();
	var PLUSSIGN = 43;
	var HYPHENMINUS = 45;
	...
	function checkInteger(offset, disallowSign) {
		...
		for (; pos < this.tokenEnd; pos++) if (!charCodeDefinitions.isDigit(this.charCodeAt(pos))) this.error("Integer is expected", pos);
	}
	...
	function consumeB() {
		...
		while (type === types.WhiteSpace || type === types.Comment) type = this.lookupType(++offset);
		if (type !== types.Number) if (this.isDelim(PLUSSIGN, offset) || this.isDelim(HYPHENMINUS, offset)) {
		...
		if (sign === 0) {
			...
			if (type !== PLUSSIGN && type !== HYPHENMINUS) this.error("Number sign is expected");
		}
	...
	function parse() {
		...
		if (this.tokenType === types.Number) {
			...
			for (; i < this.tokenEnd; i++) if (!charCodeDefinitions.isDigit(this.charCodeAt(i))) break;
		...
		if (a !== null && a.charCodeAt(0) === PLUSSIGN) a = a.substr(1);
		if (b !== null && b.charCodeAt(0) === PLUSSIGN) b = b.substr(1);
		return {
			type: "AnPlusB",
...
//#endregion
//#region node_modules/css-tree/cjs/syntax/node/Atrule.cjs
var require_Atrule = /* @__PURE__ */ __commonJSMin(((exports) => {
	var types = require_types();
	function consumeRaw() {
	...
	function isDeclarationBlockAtrule() {
		for (let offset = 1, type; type = this.lookupType(offset); offset++) {
			if (type === types.RightCurlyBracket) return true;
			if (type === types.LeftCurlyBracket || type === types.AtKeyword) return false;
		}
		return false;
	}
	var name = "Atrule";
	...
	function parse(isDeclaration = false) {
		...
		if (this.eof === false && this.tokenType !== types.LeftCurlyBracket && this.tokenType !== types.Semicolon) {
			if (this.parseAtrulePrelude) prelude = this.parseWithFallback(this.AtrulePrelude.bind(this, name, isDeclaration), consumeRaw);
	...
	function generate(node) {
		...
		else this.token(types.Semicolon, ";");
	}
	exports.generate = generate;
	exports.name = name;
	exports.parse = parse;
...
//#endregion
//#region node_modules/css-tree/cjs/syntax/node/AtrulePrelude.cjs
var require_AtrulePrelude = /* @__PURE__ */ __commonJSMin(((exports) => {
	var types = require_types();
	...
	function parse(name) {
		...
		this.skipSC();
		if (this.eof !== true && this.tokenType !== types.LeftCurlyBracket && this.tokenType !== types.Semicolon) this.error("Semicolon or block is expected");
		return {
			type: "AtrulePrelude",
...
//#endregion
//#region node_modules/css-tree/cjs/syntax/node/AttributeSelector.cjs
var require_AttributeSelector = /* @__PURE__ */ __commonJSMin(((exports) => {
	var types = require_types();
	var DOLLARSIGN = 36;
	var ASTERISK = 42;
	var EQUALSSIGN = 61;
	...
	function getAttributeName() {
		...
		if (this.isDelim(ASTERISK)) {
			expectIdent = true;
			this.next();
		} else if (!this.isDelim(VERTICALLINE)) this.eat(types.Ident);
	...
	function getOperator() {
		...
		if (code !== EQUALSSIGN && code !== TILDE && code !== CIRCUMFLEXACCENT && code !== DOLLARSIGN && code !== ASTERISK && code !== VERTICALLINE) this.error("Attribute selector (=, ~=, ^=, $=, *=, |=) is expected");
		this.next();
		if (code !== EQUALSSIGN) {
	...
	function generate(node) {
		...
		if (node.flags !== null) this.token(types.Ident, node.flags);
		this.token(types.Delim, "]");
	}
	exports.generate = generate;
	exports.name = name;
...
//#endregion
//#region node_modules/css-tree/cjs/syntax/node/Block.cjs
var require_Block = /* @__PURE__ */ __commonJSMin(((exports) => {
	...
	function consumeDeclaration() {
		if (this.tokenType === types.Semicolon) return consumeRawDeclaration.call(this, this.tokenIndex);
		const node = this.parseWithFallback(this.Declaration, consumeRawDeclaration);
		if (this.tokenType === types.Semicolon) this.next();
		return node;
	}
	var name = "Block";
	...
	function parse(isStyleBlock) {
		...
		scan: while (!this.eof) switch (this.tokenType) {
			case types.RightCurlyBracket: break scan;
			case types.WhiteSpace:
			case types.Comment:
				this.next();
				break;
			case types.AtKeyword:
				...
				break;
			default: if (isStyleBlock && this.isDelim(AMPERSAND)) children.push(consumeRule.call(this));
			else children.push(consumer.call(this));
		}
		if (!this.eof) this.eat(types.RightCurlyBracket);
		return {
			type: "Block",
			loc: this.getLocation(start, this.tokenStart),
			children
	...
	function generate(node) {
		...
		this.children(node, (prev) => {
			if (prev.type === "Declaration") this.token(types.Semicolon, ";");
		});
...
//#endregion
//#region node_modules/css-tree/cjs/syntax/node/Brackets.cjs
var require_Brackets = /* @__PURE__ */ __commonJSMin(((exports) => {
	var types = require_types();
	var name = "Brackets";
	var structure = { children: [[]] };
	function parse(readSequence, recognizer) {
		...
		if (!this.eof) this.eat(types.RightSquareBracket);
		return {
			type: "Brackets",
			loc: this.getLocation(start, this.tokenStart),
			children
...
//#endregion
//#region node_modules/css-tree/cjs/syntax/node/CDC.cjs
var require_CDC = /* @__PURE__ */ __commonJSMin(((exports) => {
	var types = require_types();
	var name = "CDC";
	var structure = [];
	function parse() {
...
//#endregion
//#region node_modules/css-tree/cjs/syntax/node/CDO.cjs
var require_CDO = /* @__PURE__ */ __commonJSMin(((exports) => {
	var types = require_types();
	var name = "CDO";
	var structure = [];
	function parse() {
...
//#endregion
//#region node_modules/css-tree/cjs/syntax/node/ClassSelector.cjs
var require_ClassSelector = /* @__PURE__ */ __commonJSMin(((exports) => {
	var types = require_types();
	var FULLSTOP = 46;
	var name = "ClassSelector";
	var structure = { name: String };
	function parse() {
...
//#endregion
//#region node_modules/css-tree/cjs/syntax/node/Combinator.cjs
...
//#endregion
//#region node_modules/css-tree/cjs/syntax/node/Comment.cjs
var require_Comment = /* @__PURE__ */ __commonJSMin(((exports) => {
	...
	function parse() {
		...
		if (end - start + 2 >= 2 && this.charCodeAt(end - 2) === ASTERISK && this.charCodeAt(end - 1) === SOLIDUS) end -= 2;
		return {
			type: "Comment",
			loc: this.getLocation(start, this.tokenStart),
...
//#endregion
//#region node_modules/css-tree/cjs/syntax/node/Condition.cjs
var require_Condition = /* @__PURE__ */ __commonJSMin(((exports) => {
	var types = require_types();
	...
	function featureOrRange(kind) {
		if (this.lookupTypeNonSC(1) === types.Ident && likelyFeatureToken.has(this.lookupTypeNonSC(2))) return this.Feature(kind);
...
//#endregion
//#region node_modules/css-tree/cjs/syntax/node/Declaration.cjs
var require_Declaration = /* @__PURE__ */ __commonJSMin(((exports) => {
	...
	function consumeValue() {
		...
		if (value.type !== "Raw" && this.eof === false && this.tokenType !== types.Semicolon && this.isDelim(EXCLAMATIONMARK) === false && this.isBalanceEdge(startValueToken) === false) this.error();
		return value;
	}
	...
	function parse() {
		...
		if (customProperty && value.type === "Value" && value.children.isEmpty) {
			for (let offset = valueStart - this.tokenIndex; offset <= 0; offset++) if (this.lookupType(offset) === types.WhiteSpace) {
		...
		if (this.eof === false && this.tokenType !== types.Semicolon && this.isBalanceEdge(startToken) === false) this.error();
		return {
			type: "Declaration",
			loc: this.getLocation(start, this.tokenStart),
			important,
	...
	function readProperty() {
		const start = this.tokenStart;
		if (this.tokenType === types.Delim) switch (this.charCodeAt(this.tokenStart)) {
			case ASTERISK:
			case DOLLARSIGN:
			case PLUSSIGN:
			case NUMBERSIGN:
			...
			case SOLIDUS:
				this.next();
				if (this.isDelim(SOLIDUS)) this.next();
				break;
		}
		if (this.tokenType === types.Hash) this.eat(types.Hash);
		else this.eat(types.Ident);
		return this.substrToCursor(start);
	}
...
//#endregion
//#region node_modules/css-tree/cjs/syntax/node/DeclarationList.cjs
var require_DeclarationList = /* @__PURE__ */ __commonJSMin(((exports) => {
	var types = require_types();
	var AMPERSAND = 38;
	function consumeRaw() {
		return this.Raw(this.consumeUntilSemicolonIncluded, true);
	...
	function parse() {
		...
		while (!this.eof) switch (this.tokenType) {
			...
			case types.AtKeyword:
				...
				break;
			default: if (this.isDelim(AMPERSAND)) children.push(this.parseWithFallback(this.Rule, consumeRaw));
			else children.push(this.parseWithFallback(this.Declaration, consumeRaw));
		}
		return {
...
//#endregion
//#region node_modules/css-tree/cjs/syntax/node/Dimension.cjs
var require_Dimension = /* @__PURE__ */ __commonJSMin(((exports) => {
	var types = require_types();
	var name = "Dimension";
	var structure = {
		value: String,
...
//#endregion
//#region node_modules/css-tree/cjs/syntax/node/Feature.cjs
var require_Feature = /* @__PURE__ */ __commonJSMin(((exports) => {
	...
	function parse(kind) {
		...
		if (this.tokenType !== types.RightParenthesis) {
			...
			switch (this.tokenType) {
				case types.Number:
					if (this.lookupNonWSType(1) === types.Delim) value = this.Ratio();
				...
				case types.Function:
					value = this.parseWithFallback(() => {
						...
						this.skipSC();
						if (this.isDelim(SOLIDUS)) this.error();
						return res;
					}, () => {
						return this.Ratio();
					});
		...
		if (!this.eof) this.eat(types.RightParenthesis);
		return {
			type: "Feature",
			loc: this.getLocation(start, this.tokenStart),
			kind,
...
//#endregion
//#region node_modules/css-tree/cjs/syntax/node/FeatureFunction.cjs
var require_FeatureFunction = /* @__PURE__ */ __commonJSMin(((exports) => {
	var types = require_types();
...
//#endregion
//#region node_modules/css-tree/cjs/syntax/node/FeatureRange.cjs
var require_FeatureRange = /* @__PURE__ */ __commonJSMin(((exports) => {
	var types = require_types();
	var SOLIDUS = 47;
	var LESSTHANSIGN = 60;
	var EQUALSSIGN = 61;
	...
	function readTerm() {
		this.skipSC();
		switch (this.tokenType) {
			case types.Number: if (this.isDelim(SOLIDUS, this.lookupOffsetNonSC(1))) return this.Ratio();
			else return this.Number();
			case types.Dimension: return this.Dimension();
			case types.Ident: return this.Identifier();
			case types.Function: return this.parseWithFallback(() => {
				const res = this.Function(this.readSequence, this.scope.Value);
				this.skipSC();
				if (this.isDelim(SOLIDUS)) this.error();
				return res;
...
//#endregion
//#region node_modules/css-tree/cjs/syntax/node/Function.cjs
...
//#endregion
//#region node_modules/css-tree/cjs/syntax/node/GeneralEnclosed.cjs
var require_GeneralEnclosed = /* @__PURE__ */ __commonJSMin(((exports) => {
	var types = require_types();
	...
	function parse(kind) {
		...
		if (this.tokenType === types.Function) functionName = this.consumeFunctionName();
		else this.eat(types.LeftParenthesis);
	...
	function generate(node) {
		if (node.function) this.token(types.Function, node.function + "(");
		else this.token(types.LeftParenthesis, "(");
		this.children(node);
		this.token(types.RightParenthesis, ")");
	}
	exports.generate = generate;
...
//#endregion
//#region node_modules/css-tree/cjs/syntax/node/Hash.cjs
...
//#endregion
//#region node_modules/css-tree/cjs/syntax/node/Identifier.cjs
...
//#endregion
//#region node_modules/css-tree/cjs/syntax/node/IdSelector.cjs
var require_IdSelector = /* @__PURE__ */ __commonJSMin(((exports) => {
	var types = require_types();
	var name = "IdSelector";
	var structure = { name: String };
	function parse() {
...
//#endregion
//#region node_modules/css-tree/cjs/syntax/node/Layer.cjs
...
//#endregion
//#region node_modules/css-tree/cjs/syntax/node/LayerList.cjs
var require_LayerList = /* @__PURE__ */ __commonJSMin(((exports) => {
	var types = require_types();
	var name = "LayerList";
	var structure = { children: [["Layer"]] };
	function parse() {
		const children = this.createList();
		this.skipSC();
		while (!this.eof) {
			children.push(this.Layer());
			if (this.lookupTypeNonSC(0) !== types.Comma) break;
			this.skipSC();
			this.next();
			this.skipSC();
		}
...
//#endregion
//#region node_modules/css-tree/cjs/syntax/node/MediaQuery.cjs
var require_MediaQuery = /* @__PURE__ */ __commonJSMin(((exports) => {
	var types = require_types();
	var name = "MediaQuery";
	var structure = {
	...
	function generate(node) {
		if (node.mediaType) {
			if (node.modifier) this.token(types.Ident, node.modifier);
...
//#endregion
//#region node_modules/css-tree/cjs/syntax/node/MediaQueryList.cjs
var require_MediaQueryList = /* @__PURE__ */ __commonJSMin(((exports) => {
	var types = require_types();
	var name = "MediaQueryList";
	var structure = { children: [["MediaQuery"]] };
	function parse() {
		const children = this.createList();
		this.skipSC();
		while (!this.eof) {
			children.push(this.MediaQuery());
			if (this.tokenType !== types.Comma) break;
			this.next();
		}
		return {
			type: "MediaQueryList",
...
//#endregion
//#region node_modules/css-tree/cjs/syntax/node/NestingSelector.cjs
var require_NestingSelector = /* @__PURE__ */ __commonJSMin(((exports) => {
	var types = require_types();
	var AMPERSAND = 38;
...
//#endregion
//#region node_modules/css-tree/cjs/syntax/node/Nth.cjs
var require_Nth = /* @__PURE__ */ __commonJSMin(((exports) => {
	var types = require_types();
	var name = "Nth";
	var structure = {
...
//#endregion
//#region node_modules/css-tree/cjs/syntax/node/Number.cjs
...
//#endregion
//#region node_modules/css-tree/cjs/syntax/node/Operator.cjs
var require_Operator = /* @__PURE__ */ __commonJSMin(((exports) => {
	var name = "Operator";
	var structure = { value: String };
	function parse() {
		const start = this.tokenStart;
...
//#endregion
//#region node_modules/css-tree/cjs/syntax/node/Parentheses.cjs
var require_Parentheses = /* @__PURE__ */ __commonJSMin(((exports) => {
	var types = require_types();
...
//#endregion
//#region node_modules/css-tree/cjs/syntax/node/Percentage.cjs
var require_Percentage = /* @__PURE__ */ __commonJSMin(((exports) => {
	var types = require_types();
	var name = "Percentage";
	var structure = { value: String };
	function parse() {
...
//#endregion
//#region node_modules/css-tree/cjs/syntax/node/PseudoClassSelector.cjs
var require_PseudoClassSelector = /* @__PURE__ */ __commonJSMin(((exports) => {
	var types = require_types();
	...
	function parse() {
		...
		if (this.tokenType === types.Function) {
			...
			nameLowerCase = name.toLowerCase();
			if (this.lookupNonWSType(0) == types.RightParenthesis) children = this.createList();
			...
			this.eat(types.RightParenthesis);
		} else name = this.consume(types.Ident);
		return {
	...
	function generate(node) {
		this.token(types.Colon, ":");
		if (node.children === null) this.token(types.Ident, node.name);
		else {
			this.token(types.Function, node.name + "(");
			this.children(node);
			this.token(types.RightParenthesis, ")");
...
//#endregion
//#region node_modules/css-tree/cjs/syntax/node/PseudoElementSelector.cjs
var require_PseudoElementSelector = /* @__PURE__ */ __commonJSMin(((exports) => {
	var types = require_types();
...
//#endregion
//#region node_modules/css-tree/cjs/syntax/node/Ratio.cjs
var require_Ratio = /* @__PURE__ */ __commonJSMin(((exports) => {
	var types = require_types();
	var SOLIDUS = 47;
	function consumeTerm() {
		this.skipSC();
		switch (this.tokenType) {
			case types.Number: return this.Number();
			case types.Function: return this.Function(this.readSequence, this.scope.Value);
...
//#endregion
//#region node_modules/css-tree/cjs/syntax/node/Raw.cjs
var require_Raw = /* @__PURE__ */ __commonJSMin(((exports) => {
	var types = require_types();
	function getOffsetExcludeWS() {
		if (this.tokenIndex > 0) {
			if (this.lookupType(-1) === types.WhiteSpace) return this.tokenIndex > 1 ? this.getTokenStart(this.tokenIndex - 1) : this.firstCharOffset;
		}
...
//#endregion
//#region node_modules/css-tree/cjs/syntax/node/Rule.cjs
var require_Rule = /* @__PURE__ */ __commonJSMin(((exports) => {
	...
	function consumePrelude() {
		...
		if (prelude.type !== "Raw" && this.eof === false && this.tokenType !== types.LeftCurlyBracket) this.error();
		return prelude;
	}
	var name = "Rule";
	...
	function parse() {
		...
		let prelude;
		let block;
		if (this.parseRulePrelude) prelude = this.parseWithFallback(consumePrelude, consumeRaw);
		else prelude = consumeRaw.call(this, startToken);
...
//#endregion
//#region node_modules/css-tree/cjs/syntax/node/Scope.cjs
...
//#endregion
//#region node_modules/css-tree/cjs/syntax/node/Selector.cjs
...
//#endregion
//#region node_modules/css-tree/cjs/syntax/node/SelectorList.cjs
var require_SelectorList = /* @__PURE__ */ __commonJSMin(((exports) => {
	var types = require_types();
	var name = "SelectorList";
	var walkContext = "selector";
	var structure = { children: [["Selector", "Raw"]] };
	function parse() {
		const children = this.createList();
		while (!this.eof) {
...
//#endregion
//#region node_modules/css-tree/cjs/utils/string.cjs
	...
	function encode(str, apostrophe) {
		...
		for (let i = 0; i < str.length; i++) {
			...
			if (code === quoteCode || code === REVERSE_SOLIDUS) {
				...
				if (wsBeforeHexIsNeeded && (charCodeDefinitions.isHexDigit(code) || charCodeDefinitions.isWhiteSpace(code))) encoded += " ";
				encoded += str.charAt(i);
				wsBeforeHexIsNeeded = false;
			}
		}
...
//#endregion
//#region node_modules/css-tree/cjs/syntax/node/String.cjs
var require_String = /* @__PURE__ */ __commonJSMin(((exports) => {
...
//#endregion
//#region node_modules/css-tree/cjs/syntax/node/StyleSheet.cjs
...
//#endregion
//#region node_modules/css-tree/cjs/syntax/node/SupportsDeclaration.cjs
var require_SupportsDeclaration = /* @__PURE__ */ __commonJSMin(((exports) => {
	var types = require_types();
	var name = "SupportsDeclaration";
	var structure = { declaration: "Declaration" };
	function parse() {
		const start = this.tokenStart;
		this.eat(types.LeftParenthesis);
		this.skipSC();
...
//#endregion
//#region node_modules/css-tree/cjs/syntax/node/TypeSelector.cjs
var require_TypeSelector = /* @__PURE__ */ __commonJSMin(((exports) => {
	var types = require_types();
	var ASTERISK = 42;
	var VERTICALLINE = 124;
	function eatIdentifierOrAsterisk() {
		if (this.tokenType !== types.Ident && this.isDelim(ASTERISK) === false) this.error("Identifier or asterisk is expected");
		this.next();
	}
...
//#endregion
//#region node_modules/css-tree/cjs/syntax/node/UnicodeRange.cjs
var require_UnicodeRange = /* @__PURE__ */ __commonJSMin(((exports) => {
	var types = require_types();
	var charCodeDefinitions = require_char_code_definitions();
	var PLUSSIGN = 43;
	var HYPHENMINUS = 45;
	...
	function eatHexSequence(offset, allowDash) {
		...
		for (let pos = this.tokenStart + offset; pos < this.tokenEnd; pos++) {
			...
			if (!charCodeDefinitions.isHexDigit(code)) this.error(allowDash && len !== 0 ? "Hyphen minus" + (len < 6 ? " or hex digit" : "") + " is expected" : len < 6 ? "Hex digit is expected" : "Unexpected input", pos);
	...
	function startsWith(code) {
		if (this.charCodeAt(this.tokenStart) !== code) this.error((code === PLUSSIGN ? "Plus sign" : "Hyphen minus") + " is expected");
	}
...
//#endregion
//#region node_modules/css-tree/cjs/utils/url.cjs
var require_url = /* @__PURE__ */ __commonJSMin(((exports) => {
	...
	function decode(str) {
		...
		let decoded = "";
		while (start < end && charCodeDefinitions.isWhiteSpace(str.charCodeAt(start))) start++;
		while (start < end && charCodeDefinitions.isWhiteSpace(str.charCodeAt(end))) end--;
		for (let i = start; i <= end; i++) {
			let code = str.charCodeAt(i);
			if (code === REVERSE_SOLIDUS) {
				if (i === end) {
	...
	function encode(str) {
		...
		for (let i = 0; i < str.length; i++) {
			...
			if (code === SPACE || code === REVERSE_SOLIDUS || code === QUOTATION_MARK || code === APOSTROPHE || code === LEFTPARENTHESIS || code === RIGHTPARENTHESIS) {
				...
				if (wsBeforeHexIsNeeded && charCodeDefinitions.isHexDigit(code)) encoded += " ";
				encoded += str.charAt(i);
				wsBeforeHexIsNeeded = false;
			}
		}
...
//#endregion
//#region node_modules/css-tree/cjs/syntax/node/Url.cjs
var require_Url = /* @__PURE__ */ __commonJSMin(((exports) => {
	var url = require_url();
	var string = require_string();
	var types = require_types();
...
//#endregion
//#region node_modules/css-tree/cjs/syntax/node/Value.cjs
var require_Value = /* @__PURE__ */ __commonJSMin(((exports) => {
	var name = "Value";
	var structure = { children: [[]] };
	function parse() {
		const start = this.tokenStart;
...
//#endregion
//#region node_modules/css-tree/cjs/syntax/node/WhiteSpace.cjs
...
//#endregion
//#region node_modules/css-tree/cjs/syntax/node/index.cjs
...
//#endregion
//#region node_modules/css-tree/cjs/syntax/config/lexer.cjs
...
//#endregion
//#region node_modules/css-tree/cjs/syntax/scope/default.cjs
var require_default = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	...
	function defaultRecognizer(context) {
		switch (this.tokenType) {
			case types.Hash: return this.Hash();
			case types.Comma: return this.Operator();
			case types.LeftParenthesis: return this.Parentheses(this.readSequence, context.recognizer);
			case types.LeftSquareBracket: return this.Brackets(this.readSequence, context.recognizer);
			case types.String: return this.String();
			case types.Dimension: return this.Dimension();
			case types.Percentage: return this.Percentage();
			case types.Number: return this.Number();
			case types.Function: return this.cmpStr(this.tokenStart, this.tokenEnd, "url(") ? this.Url() : this.Function(this.readSequence, context.recognizer);
			case types.Url: return this.Url();
			case types.Ident: if (this.cmpChar(this.tokenStart, U) && this.cmpChar(this.tokenStart + 1, PLUSSIGN)) return this.UnicodeRange();
			else return this.Identifier();
			case types.Delim: {
				const code = this.charCodeAt(this.tokenStart);
				if (code === SOLIDUS || code === ASTERISK || code === PLUSSIGN || code === HYPHENMINUS) return this.Operator();
				if (code === NUMBERSIGN) this.error("Hex or identifier is expected", this.tokenStart + 1);
				break;
			}
		}
	}
...
//#endregion
//#region node_modules/css-tree/cjs/syntax/scope/atrulePrelude.cjs
var require_atrulePrelude = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	module.exports = { getNode: require_default() };
}));
//#endregion
//#region node_modules/css-tree/cjs/syntax/scope/selector.cjs
var require_selector = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	...
	function getNode() {
		switch (this.tokenType) {
			case types.LeftSquareBracket: return this.AttributeSelector();
			case types.Hash: return this.IdSelector();
			case types.Colon: if (this.lookupType(1) === types.Colon) return this.PseudoElementSelector();
			else return this.PseudoClassSelector();
			case types.Ident: return this.TypeSelector();
			case types.Number:
			case types.Percentage: return this.Percentage();
			case types.Dimension:
...
//#endregion
//#region node_modules/css-tree/cjs/syntax/function/expression.cjs
...
//#endregion
//#region node_modules/css-tree/cjs/syntax/function/var.cjs
var require_var = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	var types = require_types();
	function varFn() {
		const children = this.createList();
		this.skipSC();
		...
		if (this.tokenType === types.Comma) {
			...
			if (value.type === "Value" && value.children.isEmpty) {
				for (let offset = startIndex - this.tokenIndex; offset <= 0; offset++) if (this.lookupType(offset) === types.WhiteSpace) {
					value.children.appendData({
						type: "WhiteSpace",
						loc: null,
						value: " "
...
//#endregion
//#region node_modules/css-tree/cjs/syntax/scope/value.cjs
var require_value = /* @__PURE__ */ __commonJSMin(((exports, module) => {
...
//#endregion
//#region node_modules/css-tree/cjs/syntax/scope/index.cjs
...
//#endregion
//#region node_modules/css-tree/cjs/syntax/atrule/container.cjs
...
//#endregion
//#region node_modules/css-tree/cjs/syntax/atrule/font-face.cjs
var require_font_face = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	module.exports = { parse: {
		prelude: null,
		block() {
...
//#endregion
//#region node_modules/css-tree/cjs/syntax/atrule/import.cjs
var require_import = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	...
	function parseWithFallback(parse, fallback) {
		return this.parseWithFallback(() => {
			try {
				...
				this.skipSC();
				if (this.lookupNonWSType(0) !== types.RightParenthesis) this.error();
			}
	...
	module.exports = { parse: {
		prelude() {
			...
			this.skipSC();
			if (this.tokenType === types.Ident && this.cmpStr(this.tokenStart, this.tokenEnd, "layer")) children.push(this.Identifier());
			else if (this.tokenType === types.Function && this.cmpStr(this.tokenStart, this.tokenEnd, "layer(")) children.push(this.Function(null, parseFunctions));
			this.skipSC();
			if (this.tokenType === types.Function && this.cmpStr(this.tokenStart, this.tokenEnd, "supports(")) children.push(this.Function(null, parseFunctions));
			if (this.lookupNonWSType(0) === types.Ident || this.lookupNonWSType(0) === types.LeftParenthesis) children.push(this.MediaQueryList());
			return children;
		},
		block: null
	} };
...
//#endregion
//#region node_modules/css-tree/cjs/syntax/atrule/layer.cjs
...
//#endregion
//#region node_modules/css-tree/cjs/syntax/atrule/media.cjs
...
//#endregion
//#region node_modules/css-tree/cjs/syntax/atrule/nest.cjs
var require_nest = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	module.exports = { parse: {
		prelude() {
...
//#endregion
//#region node_modules/css-tree/cjs/syntax/atrule/page.cjs
var require_page = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	module.exports = { parse: {
		prelude() {
			return this.createSingleNodeList(this.SelectorList());
		},
...
//#endregion
//#region node_modules/css-tree/cjs/syntax/atrule/scope.cjs
...
//#endregion
//#region node_modules/css-tree/cjs/syntax/atrule/starting-style.cjs
var require_starting_style = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	module.exports = { parse: {
		prelude: null,
		block(nested = false) {
			return this.Block(nested);
...
//#endregion
//#region node_modules/css-tree/cjs/syntax/atrule/supports.cjs
var require_supports = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	module.exports = { parse: {
		prelude() {
...
//#endregion
//#region node_modules/css-tree/cjs/syntax/atrule/index.cjs
var require_atrule = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	module.exports = {
		...
		page: require_page(),
		scope: require_scope(),
		"starting-style": require_starting_style(),
		supports: require_supports()
	};
}));
//#endregion
//#region node_modules/css-tree/cjs/syntax/pseudo/lang.cjs
		...
		loop: while (!this.eof) {
			switch (this.tokenType) {
				...
				case types.Comma:
					children.push(this.Operator());
					break;
				case types.RightParenthesis: break loop;
...
//#endregion
//#region node_modules/css-tree/cjs/syntax/pseudo/index.cjs
var require_pseudo = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	...
	var langList = { parse: lang.parseLanguageRangeList };
	var nth = { parse() {
...
//#endregion
//#region node_modules/css-tree/cjs/syntax/node/index-parse.cjs
var require_index_parse = /* @__PURE__ */ __commonJSMin(((exports) => {
	var AnPlusB = require_AnPlusB();
	var Atrule = require_Atrule();
	var AtrulePrelude = require_AtrulePrelude();
	var AttributeSelector = require_AttributeSelector();
...
//#endregion
//#region node_modules/css-tree/cjs/syntax/config/parser.cjs
var require_parser = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	module.exports = {
		...
		scope: require_scope$1(),
		atrule: require_atrule(),
		pseudo: require_pseudo(),
		node: require_index_parse()
	};
}));
//#endregion
//#region node_modules/css-tree/cjs/syntax/config/walker.cjs
var require_walker = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	module.exports = { node: require_node() };
}));
//#endregion
//#region node_modules/css-tree/cjs/syntax/index.cjs
...
//#endregion
//#region node_modules/css-tree/package.json
var package_exports = /* @__PURE__ */ __exportAll({
	...
	unpkg: () => unpkg,
	version: () => version
}), name, version, description, author, repository, keywords, type, module$1, main, exports$1, browser, unpkg, jsdelivr, scripts, dependencies, devDependencies, engines, files, package_default;
...
//#endregion
//#region node_modules/css-tree/cjs/version.cjs
var require_version = /* @__PURE__ */ __commonJSMin(((exports) => {
	var { version } = (init_package(), __toCommonJS(package_exports).default);
	exports.version = version;
}));
//#endregion
//#region node_modules/css-tree/cjs/definition-syntax/index.cjs
var require_definition_syntax = /* @__PURE__ */ __commonJSMin(((exports) => {
	var SyntaxError = require_SyntaxError();
	var generate = require_generate();
	var parse = require_parse();
	var walk = require_walk();
...
//#endregion
//#region node_modules/css-tree/cjs/utils/clone.cjs
var require_clone = /* @__PURE__ */ __commonJSMin(((exports) => {
	...
	function clone(node) {
		...
		for (const key of Object.keys(node)) {
			let value = node[key];
			if (value) {
				if (Array.isArray(value) || value instanceof List.List) value = value.map(clone);
...
//#endregion
//#region node_modules/css-tree/cjs/utils/ident.cjs
var require_ident = /* @__PURE__ */ __commonJSMin(((exports) => {
	var charCodeDefinitions = require_char_code_definitions();
	var utils = require_utils();
	var REVERSE_SOLIDUS = 92;
	function decode(str) {
	...
	function encode(str) {
		...
		for (let i = 0; i < str.length; i++) {
			...
			if (charCodeDefinitions.isName(code)) encoded += str.charAt(i);
...
//#endregion
//#region node_modules/css-tree/cjs/index.cjs
var require_cjs = /* @__PURE__ */ __commonJSMin(((exports) => {
	...
	var TokenStream = require_TokenStream();
	var OffsetToLocation = require_OffsetToLocation();
	var { tokenize, parse, generate, lexer, createLexer, walk, find, findLast, findAll, toPlainObject, fromPlainObject, fork } = index$1;
