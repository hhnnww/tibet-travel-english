...
import { Ct as require_client, D as init_dist_es$1, Dt as init_dist_es$2, F as init_dist_es, I as chain, Ot as HttpRequest, P as getProfileName, R as CredentialsProviderError, j as readFile, k as parseKnownFiles } from "./client-s3+[...].mjs";
...
var resolveCredentialSource = (credentialSource, profileName, logger) => {
	const sourceProvidersMap = {
		EcsContainer: async (options) => {
			...
			const { fromContainerMetadata } = await import("../@smithy/credential-provider-imds+[...].mjs").then((n) => (n.n(), n.t));
			logger?.debug("@aws-sdk/credential-provider-ini - credential_source is EcsContainer");
			return async () => chain(fromHttp(options ?? {}), fromContainerMetadata(options))().then(setNamedProvider);
		},
		Ec2InstanceMetadata: async (options) => {
			logger?.debug("@aws-sdk/credential-provider-ini - credential_source is Ec2InstanceMetadata");
			const { fromInstanceMetadata } = await import("../@smithy/credential-provider-imds+[...].mjs").then((n) => (n.n(), n.t));
			return async () => fromInstanceMetadata(options)().then(setNamedProvider);
		},
		Environment: async (options) => {
			logger?.debug("@aws-sdk/credential-provider-ini - credential_source is Environment");
			const { fromEnv } = await import("./client-s3+[...].mjs").then((n) => n.m);
...
var resolveProcessCredentials = async (options, profile) => import("./credential-provider-process+[...].mjs").then((n) => n.t).then(({ fromProcess }) => fromProcess({
	...options,
	profile
...
var resolveWebIdentityCredentials = async (profile, options, callerClientConfig) => import("./credential-provider-web-identity+[...].mjs").then((n) => n.t).then(({ fromTokenFile }) => fromTokenFile({
